...
bool initLabels(std::vector < vector<Lexem *> > &infix) {
	int flag = 0;
	cout << "jsize" << infix.size() << endl;
	for (int row = 0; infix.size(); row++) {
		for (int i = 1; i < infix[row].size(); i++) {
					cout << "meow-1 " << i  << " " << row <<  endl;
